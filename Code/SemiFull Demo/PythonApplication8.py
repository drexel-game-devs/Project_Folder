...
class BulletBoss2(pygame.sprite.Sprite):
    ...
    #Call superclass constructor, will take a player reference
    def __init__(self, player, bullet_list, sprite_list, screen,enemy):
        super().__init__()
    ...
    def update(self):
        ...
        #right down
        if(self.enemy.rect.x > self.player.rect.x and self.enemy.rect.y > self.player.rect.y):
           
            self.rect.x += self.vel
            self.rect.y += self.Vvel
        #left down
        elif(self.enemy.rect.x < self.player.rect.x and self.enemy.rect.y > self.player.rect.y):
    ...
    while not done:
        if TimetoKillPlayer == 15 or BossKill == 5:
            ...
            if bint % 60 == 0 and bint == 60:   
                ...
                bint = 0
                bossBullet = BulletBoss(boss1, bullet_list2, active_sprite_list, screen,player)
                bossBullet = BulletBoss2(boss1, bullet_list2, active_sprite_list, screen,player)
            ...
            for bullet in bullet_list:
                ...
                if bullet.rect.x < 0 or bullet.rect.x > SCREEN_WIDTH + 1:
                    bullet_list.remove(bullet)
                    active_sprite_list.remove(bullet)
                if bullet.rect.x in  range(boss1.rect.x-5, boss1.rect.x+100) and bullet.rect.y in range(boss1.rect.y, boss1.rect.y+200):
                    bullet_list.remove(bullet)
                    active_sprite_list.remove(bullet)
                    BossKill += 1
                    print(BossKill)
                ...
                if i.movement == 'right':
                    ...
                    if player.rect.x in range(i.rect.x-30, i.rect.x + 600) and (player.rect.y in range(i.rect.y-40, i.rect.y + 140) or newcor in range(i.rect.y-60, i.rect.y + 140)):
                        ...
                        if intervals % 60 == 0 and intervals == 60:
                            ...
                            bullet1 = Bullet(i, bullet_list1, active_sprite_list, screen)#Shooting!!
                            
                            for bullet1 in bullet_list1:
                                #see if a bullet hits a platform
                                block_hit_list1 = pygame.sprite.spritecollide(bullet1, i.level.platform_list, False)
                             
                                for block in block_hit_list1:                                   
                                    #update both lists
                                    bullet_list1.remove(bullet1)
                                    active_sprite_list.remove(bullet1)
                                #print(bullet1.rect.x)
                                if bullet1.rect.x < 0 or bullet1.rect.x > SCREEN_WIDTH + 1:
                                    bullet_list1.remove(bullet1)
                                    active_sprite_list.remove(bullet1)                 
                                if bullet1.rect.x in range(player.rect.x-20, player.rect.x+20) and bullet1.rect.y in range(player.rect.y-10,player.rect.y+50):
                                    bullet_list1.remove(bullet1)
                                    active_sprite_list.remove(bullet1)
                                    TimetoKillPlayer += 1                                   
                                    print(TimetoKillPlayer)                                                                       
                    ...
                    if player.rect.x in range(i.rect.x - 400, i.rect.x+30) and (player.rect.y in range(i.rect.y-40, i.rect.y + 130) or newcor in range(i.rect.y-60, i.rect.y + 140)):
                        ...
                        if intervals1 % 60 == 0 and intervals1 == 60:
                            intervals1 = 0
                            bullet1 = Bullet(i, bullet_list1, active_sprite_list, screen)
                            for bullet1 in bullet_list1:
                                ...
                                if bullet1.rect.x < 0 or bullet1.rect.x > SCREEN_WIDTH + 1:
                                    bullet_list1.remove(bullet1)
                                    active_sprite_list.remove(bullet1)
                                if bullet1.rect.x in range(player.rect.x-10, player.rect.x+10) and bullet1.rect.y in range(player.rect.y-10,player.rect.y+50):
                                    bullet_list1.remove(bullet1)
                                    active_sprite_list.remove(bullet1)
                                    TimetoKillPlayer += 1
                                    print(TimetoKillPlayer)
